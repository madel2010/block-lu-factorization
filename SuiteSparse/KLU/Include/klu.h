...
#ifdef BLOCKM
klu_B_numeric *klu_B_factor      /* returns KLU_OK if OK, < 0 if error */
(
     /* inputs, not modified */
     int Ap [ ],        /* size n+1, column pointers */
     int Ai [ ],        /* size nz, row indices */
     BMatrix::MWrap<double>* Ax,     /* size 2*nz, numerical values (real,imag pairs) */
     klu_symbolic *Symbolic,
     klu_common *Common
) ;
...
(
     ...
     /* right-hand-side on input, overwritten with solution to Ax=b on output */
     BMatrix::MWrap<double>* B,           /* size 2*ldim*nrhs */
     klu_common *Common
) ;
#endif
...
(
    ...
    /* right-hand-side on input, overwritten with solution to Ax=b on output */
    #ifdef BLOCKM
    BMatrix::MWrap<double> B [ ],           /* size ldim*nrhs */
    #else
    double B [ ],           /* size ldim*nrhs */
    #endif
    klu_common *Common
